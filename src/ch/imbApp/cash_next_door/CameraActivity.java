...
//FIXME wider ausbauen
    	hiddenCashMachines = AutomatenLoader.getBankomaten(myLoc, 42);
...
//            		lon.setText(String.valueOf(msg.getData().getDouble("longitude")));
    ...
    private void moved() {
    	...
    	if(lastRefresh.before(new Date())) {
    		...
    		lastRefresh = new Date(lastRefresh.getTime()+300);//"nur" alle 0.3 sec einen refresh durchführen
...
//		        	machine.setDirection(CalcAngle.calcAngle(myDirection, myLoc, machine.getLocation()));
    ...
    /**
     * Position der verfuegbaren View wird angepasst
     * ist der Bankomat nicht mehr sichtbar, wird der Record zu den unsichtbaren gelegt
     * und die TextView wieder freigegeben. 
     * @param machine
     */
...
//    		bankOmat.setLayoutParams(params);
    ...
    /**
     * Bei einer verfuegbaren Textview wird diese mit dem Record abgefuellt, sofern der Record in der gewuenschten Range ist. 
     * @param machine
     */
	...
	private void displayText(BankOmat machine, TextView bankOmat, double totDir) {
		int width = (int) (xWith /54.8);
		bankOmat.setText(machine.getBank()+"\n"+machine.getDistance());
		MarginLayoutParams params=(MarginLayoutParams )bankOmat.getLayoutParams();
		params.leftMargin = (int) (width*totDir+xWith/2);
		bankOmat.setLayoutParams(params);
